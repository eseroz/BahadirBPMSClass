...
class BTriggers
{
    ...
    public function PROFORMA_OLUSTURMA_SURECI_TALEP_DEGERLENDIR_AFTER_TRIGGER($PROCESS_UID, $PM_CASE_ID, $PM_USER_UID, $PARENT_MSSQL_CASE_ID){
        ...
        foreach ($bahadir->mssqlDb->Select("SELECT *FROM TALEP_SATIR WHERE SIPARIS_ID = $SIPARIS_ID") as $TALEP_SATIR)
        {
            ...
            if($OZEL_URETIM == 1){
                ...
                //$UPDATE_TALEP_SATIR = $bahadir->mssqlDb->ExecQuery("UPDATE TALEP_SATIR SET STOK_KART_ID = $STOK_KART_ID WHERE ID = $TALEP_SATIR_ID");
    ...
    public function REFERANS_HAZIRLAMA_SURECI_TEKNIK_RESMI_ONAYLA_AFTER2($PROCESS_UID, $APP_UID, $MSSQL_CASE_ID)
    {
        $bahadir = new Bahadir();
        $CASE_ID = $MSSQL_CASE_ID > 0 ? $MSSQL_CASE_ID : $bahadir->mssqlDb->GET_MSSQL_CASE_ID($PROCESS_UID, $APP_UID, null, false);
        $GET_CASE_ROW = $bahadir->mssqlDb->GET_CASE_ROW($CASE_ID);
        $PM_CRT_USER_UID = $GET_CASE_ROW["PM_CRT_USER_UID"];
        return $PM_CRT_USER_UID;
    }

    public function REFERANS_HAZIRLAMA_SURECI_TALEP_OLUSTUR_AFTER($PROCESS_UID, $PM_CASE_ID, $CASE_TYPE, $PM_USER_UID)
    {
        $bahadir = new Bahadir();
        $MSSQL_CASE_ID_AND_DOC_NUMBER = $bahadir->mssqlDb->GET_MSSQL_CASE_ID_AND_DOC_NUMBER($PROCESS_UID, $PM_CASE_ID, $CASE_TYPE, $PM_USER_UID, "REFERANS TALEP", false);
